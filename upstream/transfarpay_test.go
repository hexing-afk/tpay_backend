...
func GetTransfarObj() Upstream {
	upMerchantNo := "6688531146016889"
	jsonStr := `{
    "Host": "https://openapitest.tf56.com/service/api",
	"PfxFile": "D:\\doc\\对接资料\\传化\\商户资料\\海南拂蓉建材有限公司\\zhengshuhainan.pfx",
	"PfxSecret": "baibai88",
	"AppId": "3657799"
}`
	obj, err := NewTransfarPay(upMerchantNo, jsonStr)
	if err != nil {
...
func TestT(t *testing.T) {
	a, b, c := getKeys("D:\\doc\\对接资料\\传化\\商户资料\\海南拂蓉建材有限公司\\zhengshuhainan.pfx", "baibai88")
	t.Logf("%v,%v,%v", a, b, c)
}

func TestTransfarPay_QPaySign(t *testing.T) {
	up := GetTransfarObj()
	...
	req := &QPaySignRequest{
		...
		BankCardInfo: BankCardInfo{
			BankCardNo:    "65465462184684",
			BankCardName:  "asdfasf",
			BankCardIdNo:  "2342342425",
			BankCardPhone: "1231312314",
			// 信用卡必传
			BankCardCvv2: "",
			// 信用卡必传
			BankCardExpireDate: "",

			ClientIp:     "112.67.208.148",
			IsCreditCard: false,
		},
	}
	resp, err := up.(*TransfarPay).QPaySign(req)
	if err != nil {
		t.Errorf("err=%v", err)
		return
	}
...
func TestTransfarPay_PayOrderQuery(t *testing.T) {
	up := Get777PayObj()
...
func TestTransfarPay_Transfer(t *testing.T) {
	up := Get777PayObj()
...
func TestTransfarPay_TransferOrderQuery(t *testing.T) {
	up := Get777PayObj()
...
func TestTransfarPay_TransferOrderQuery2(t *testing.T) {
...
func TestTransfarPay_Sign(t *testing.T) {
...
func TestTransfarPay_CheckSign(t *testing.T) {
	...
	if reqData.IsCancel == TransfarPayCancelled {
