...
// 创建admin账号密码
func CreateAdminPassword(plainPassword string) string {
	key1 := "Yo6m$*9b"
	key2 := "Qpd@k.3p8"
...
// 创建商户支付密码
func CreateMerchantPayPassword(plainPassword string) string {
	key1 := "Yay6m$*9b"
	key2 := "bQpd@k.3p8x"
...
func LoginTokenParse(loginToken string) (int64, string, error) {
	...
	if err != nil {
		return 0, "", errors.New(fmt.Sprintf("loginToken 错误, userId转int64发生错误, err=[%v]", err))
	}
