...
func (l *LoginLogic) Login(req types.LoginReq) (*types.LoginReply, error) {
	...
	*/
	...
	// 通过用户名查找用户
	merchant, err := model.NewMerchantModel(l.svcCtx.DbEngine).FindOneByUsername(req.Username)
	if err != nil {
	...
	if err != nil {
		l.Errorf("登录失败:登录密码解密发生错误,err:%v, password:%v", err, req.Password)
		return nil, common.NewCodeError(common.SysDBErr)
	}
	//l.Infof("password:%v, plainPassword=[%v]", req.Password, plainPassword)

	// 比对密码
	if merchant.Password != common.CreateMerchantPassword(plainPassword) {
		l.Error("登录失败:密码错误")
		return nil, common.NewCodeError(common.LoginPwdFailed)
	}

	if merchant.Status != model.MerchantStatusEnable {
		l.Errorf("登录失败:账号[%v]状态为[%v],不允许不是启用状态的账号登录", merchant.Id, merchant.Status)
		return nil, common.NewCodeError(common.AccountDisable)
	}

	//删除可能存在的旧token
	if err := l.svcCtx.RedisSession.CleanOtherLogined(merchant.Id); err != nil {
		l.Error("登录失败:删除旧的redis失败:%v", err)
		return nil, common.NewCodeError(common.UserLoginFailed)
	}

	token, err := l.svcCtx.RedisSession.Login(merchant.Id)
	if err != nil {
		l.Error("登录失败:存入redis失败:%v", err)
		return nil, common.NewCodeError(common.UserLoginFailed)
	}

	loginToken := common.LoginTokenGenerate(merchant.Id, token)

	currencyData, err := model.NewCurrencyModel(l.svcCtx.DbEngine).FindByCurrency(merchant.Currency)
	if err != nil {
		l.Error("查询币种失败:%v", err)
		return nil, common.NewCodeError(common.UserLoginFailed)
	}

	cData := types.CurrencyData{
		Currency:        currencyData.Currency,
		Symbol:          currencyData.Symbol,
		IsDivideHundred: currencyData.IsDivideHundred,
	}
